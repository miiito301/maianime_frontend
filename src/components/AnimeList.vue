...
const getImageFromGoogleBooks = async (title) => {
  const url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(title)}+subject:comic&langRestrict=ja&printType=books&orderBy=relevance&maxResults=1`
  try {
